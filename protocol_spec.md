...
| **การตรวจสอบสิทธิ์** | ใช้ชื่อผู้เล่น (player name) เท่านั้น ไม่มี password |
...
### 6.2 การลงทะเบียนผู้เล่น (Player Registration)
...
หากชื่อถูกใช้งานแล้ว:
```
C: HELO PlayerOne\r\n
S: 407 Name already taken\r\n
```
...
เมื่อมีผู้เล่นอีกคนเข้าคิวเกมชนิดเดียวกัน server จะจับคู่ทั้งสองและแจ้ง:
```
-- ผู้เล่น 1 (เริ่มก่อน) --
S: 301 Your turn [board_state]\r\n

-- ผู้เล่น 2 --
S: 302 Opponent's turn [board_state]\r\n
```
...
-- server แจ้งทั้งคู่ --
S→P1: 301 Your turn [board_state]\r\n
S→P2: 302 Opponent's turn [board_state]\r\n
```
...
หลังจากตอบกลับ `204` server จะส่ง push message ไปยังคู่ต่อสู้:
```
S→opponent: 301 Your turn [board_state]\r\n
```
...
Client ควรตรวจสอบเบื้องต้นก่อนส่ง MOVE แต่ server เป็น **ผู้ตัดสินที่มีอำนาจสูงสุด**:
...
3. หากผู้เล่นที่ตัดการเชื่อมต่อกลับมาเชื่อมต่อและส่ง `HELO <same_name>` ภายใน timeout:
   ```
   C: HELO PlayerOne\r\n
...
-- server แจ้งทั้งคู่ เริ่มเกมใหม่ --
S→P1: 301 Your turn [board_state]\r\n
S→P2: 302 Opponent's turn [board_state]\r\n
```
...
|--------|--------|--------|---------|
| `HELO` | C→S | `HELO <name>\r\n` | ลงทะเบียน/เรียกคืน session ด้วยชื่อผู้เล่น |
...
| 201 | Session created | ลงทะเบียน/เรียกคืน session สำเร็จ |
| 202 | Room created | สร้างห้องสำเร็จ argument คือรหัสห้อง |
| 203 | Joined room | เข้าร่วมห้องสำเร็จ |
...
| 301 | Your turn | ถึงตาคุณเดิน พร้อม board_state |
| 302 | Opponent's turn | รอคู่ต่อสู้เดิน พร้อม board_state |
| 303 | Game over Win | คุณชนะ พร้อม board_state สุดท้าย |
| 304 | Game over Loss | คุณแพ้ พร้อม board_state สุดท้าย |
| 305 | Game over Draw | เสมอ พร้อม board_state สุดท้าย |
...
| รหัส | Phrase | ความหมาย |
|------|--------|---------|
| 400 | Bad request | รูปแบบข้อความไม่ถูกต้อง |
| 401 | Not registered | ยังไม่ได้ส่ง HELO ก่อน |
| 402 | Room not found | ไม่พบรหัสห้องที่ระบุ |
| 403 | Room full | ห้องมีผู้เล่นครบ 2 คนแล้ว |
| 404 | Not your turn | ส่ง MOVE ในตาของคู่ต่อสู้ |
| 405 | Invalid move | การเดินที่ส่งมาไม่ถูกกฎเกม |
| 406 | Game not started | ส่ง MOVE ก่อนเกมเริ่ม |
| 407 | Name already taken | ชื่อผู้เล่นนี้ถูกใช้อยู่แล้ว |
| 408 | Unknown game type | ระบุชนิดเกมไม่ถูกต้อง |
...
|--------|-------------------|-------------------|
| HELO | 201 | 407, 501 |
| QUEUE | 300 | 401, 408, 503 |
| MAKE | 202 | 401, 408, 503 |
| JOIN | 203 | 401, 402, 403, 503 |
| MOVE | 204 | 401, 404, 405, 406, 501 |
| PONG | 206 | — |
| REMATCH | 200 | 401, 503 |
| ACCEPT | 205 | 401, 503 |
| DECLINE | 200 | 401, 503 |
...
C: MOVE 0,1\r\n
S: 204 Move accepted\r\n
                                     S: 301 Your turn X,X,.|.,O,.|.,.,.\r\n

                                     C: MOVE 2,2\r\n
                                     S: 204 Move accepted\r\n
S: 301 Your turn X,X,.|.,O,.|.,.,O\r\n
...
S: 301 Your turn [board]\r\n         S: 302 Opponent's turn [board]\r\n
...
### สถานการณ์ที่ 4: การตัดการเชื่อมต่อและ Reconnect
...
[Bob reconnect และส่ง HELO อีกครั้ง]
                                     C: HELO Bob\r\n
                                     S: 201 Session restored [board_state]\r\n
S: 301 Your turn [board_state]\r\n
[เกมดำเนินต่อ]
...
-- ส่ง MOVE ก่อน HELO --
C: MOVE 1,1\r\n
S: 401 Not registered\r\n

-- ส่ง MOVE ผิดตา --
C: MOVE 2,2\r\n
S: 404 Not your turn\r\n

-- เดินในช่องที่มีหมากแล้ว --
C: MOVE 0,0\r\n
S: 405 Invalid move\r\n

-- รหัสห้องไม่มี --
C: JOIN ZZZZZZ\r\n
S: 402 Room not found\r\n
```
